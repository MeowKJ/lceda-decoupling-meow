...
- 点击“选择器件”会暂时隐藏去耦喵并打开嘉立创 EDA 底部原生器件库，点中器件后自动返回并回填；
- 主电容与引脚去耦电容可分别指定器件，也可点击“用当前”采用原生库已经高亮的器件；
- 生成过程失败时自动回滚本次已创建图元；
...
```text
build/dist/lceda-decoupling-meow_v0.5.1.eext
```
...
7. 点击主电容的“选择器件”，去耦喵会暂时隐藏并打开编辑器底部原生器件库；搜索并点中器件后窗口会自动返回并回填；
8. 同样为引脚去耦电容选择器件；若要沿用原生库当前已高亮的器件，直接点击“用当前”；
