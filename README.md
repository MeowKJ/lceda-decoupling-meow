...
- 点击一次“逐组放置 N 个电源域”后，插件按电源域依次把锚点电容黏到鼠标；
- 用户每点击一次就确定一组的位置，该组生成完成后自动进入下一组，各网络位置完全由用户安排；
...
```text
build/dist/lceda-decoupling-meow_v0.8.1.eext
```
...
7. 点击“逐组放置 N 个电源域”；
8. 根据鼠标提示依次点击每个电源域的位置；每次点击会生成当前完整电容组，然后自动切换到下一组。

## 生成方式

- 不在芯片电源引脚创建标签；
- 用户为每个电源域单独点击一个锚点，像连续粘贴一样逐组安排位置；
...
- 官方鼠标接口每次只预览当前组的锚点电容；点击后立即生成该组完整阵列，再让下一组锚点跟随鼠标；
