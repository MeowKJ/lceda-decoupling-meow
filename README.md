...
- 同一芯片已有去耦喵生成记录时，主操作改为“替换现有去耦”，避免反复测试叠加整批电容；
...
```text
build/dist/lceda-decoupling-meow_v0.6.3.eext
```
...
7. 点击“放置 N 个电源域”；若当前芯片已有插件生成结果，按钮会显示“替换现有去耦”；
8. 在画布上点击一个锚点，插件会一次完成全部已选择网络的连接式电容阵列。

生成后，顶部会出现“生成记录”，可以删除单个电容、删除整个电源域或撤销整批生成。
...
- 较短电容通过上下竖直支路接入连续母线，每个电源域只创建一个 `GND` 标志；
- 不移动芯片，不检查或删除用户已有器件；
- 每批结果独立记录图元 ID，替换和删除操作只作用于去耦喵自己创建的内容。

## 当前限制

- 只处理一个选中芯片；
- 文字左移只对本次新建电容生效；旧版已经生成但未被插件记录的孤立图元不会被自动改写；
