...
1. 🐾 **右键召唤**：在画布中右键芯片，选择“去耦喵 → 生成去耦”；
2. 👀 **喵一眼电源域**：确认自动识别的电源网络，需要时用“调整引脚”增删或重新分组；
3. 🧶 **挑选电容**：分别选择主容与去耦器件，容量和器件习惯会自动记住；
4. ✅ **决定要不要主容**：不需要主电容的网络，直接取消总览行的小勾；
5. 🐈 **整组放下**：点击“整块放置”，每次鼠标点击落下一整个电容母线组；
6. ⎋ **随时收爪**：跟随鼠标期间按 `Esc`，立即停止后续放置并清理当前未完成的组。

> 喵喵提示：去耦喵只生成原理图连接；PCB 上仍要把去耦电容尽量靠近芯片电源引脚。
...
```text
build/dist/lceda-decoupling-meow_v0.9.7.eext
```
