...
1. 预先缓存当前图元 ID，并在用户点击“放置”或“替换现有去耦”的同步阶段立即调用 `placeComponentWithMouse()`，将整批首个电容黏到鼠标；
...
7. 新批次完成后逐图元删除同一文档、同一芯片的旧插件批次；删除失败的批次继续保留在历史记录中。
8. 找出上下引脚跨度最大的电容，以其上端和下端分别确定电源/GND 母线位置，并在左侧放置电源标签和单个 GND 标识；
9. 较短电容的上下引脚分别通过竖直支路接入公共母线；
10. 不在芯片电源引脚添加标签，不修改芯片本体；
11. 重新读取客户端合并后的稳定导线 ID，分别保存器件 ID 和导线 ID。
...
- 较短电容通过上下竖直支路接入母线，只在 GND 母线一端放一个 GND 标识；
